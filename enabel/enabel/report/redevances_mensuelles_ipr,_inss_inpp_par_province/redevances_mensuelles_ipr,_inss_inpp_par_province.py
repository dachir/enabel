...
def get_columns(filters):
	columns = [
		{ "label": _("Province"), "fieldtype": "Link",	"fieldname": "branch", "options": "Branch", "width": 100, },
        ...
        FROM(
            ...
            FROM
                (SELECT s.pay_period, s.name, e.branch, e.projet, p.project_name,s.currency,
                    ...
                    SUM(CASE WHEN d.parentfield = 'earnings' AND d.abbr = 'P' THEN amount ELSE 0 END) + SUM(CASE WHEN d.parentfield = 'earnings' AND d.abbr = 'P_P' THEN amount ELSE 0 END) +
                    SUM(CASE WHEN d.parentfield = 'earnings' AND d.abbr = 'P_F_P' THEN amount ELSE 0 END)
                    AS base,
